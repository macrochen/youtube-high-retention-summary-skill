// ==UserScript==
// @name         Gemini YouTube Summary Automator V12 (Strict Filename)
// @namespace    http://tampermonkey.net/
// @version      12.0
// @description  严苛过滤所有中英文标点，防止触发浏览器全局路径替换机制
    ...
    function downloadMarkdown(text) {
        ...
        if (lines.length > 0) {
            ...
            if (firstLine.length > 0 && firstLine.length < 50) {
                // 【V12 核心修改】极度严苛的正则表达式，只保留汉字、字母、数字和空格
                title = firstLine.replace(/[^\u4e00-\u9fa5a-zA-Z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
            }
        }
        ...
        console.log(`[下载追踪] 准备使用 GM_download 下载到: youtube-summeries/${title}.md`);
        GM_download({
            url: url,
            name: `youtube-summeries/${title}.md`,
            saveAs: false,
            onload: function() { 
                console.log("[下载追踪] GM_download 成功触发！");
                URL.revokeObjectURL(url); 
            },
            onerror: function(err) {
                console.error("[下载追踪] GM_download 失败:", err);
